...
- prompt_breakdown with chat_history AND chat_history_count (FIXED!)
...
def create_prompt_breakdown_from_messages(messages: list) -> dict:
    ...
    UPDATED: Now includes chat_history_count for conversation tracking!
    ...
    chat_history_count = 0  # ✅ ADDED: Track message count
    ...
    for msg in messages:
        ...
        if role == "system":
            ...
            if user_message is not None:
                ...
                chat_history_count += 1  # ✅ ADDED: Increment count
    ...
    return create_prompt_breakdown(
        ...
        chat_history_count=chat_history_count if chat_history else 0,  # ✅ ADDED: Pass count
    )
...
class ScenarioRunner:
    ...
    async def _execute_turn(
        ...
        - prompt_breakdown with full chat_history AND chat_history_count (FIXED!)
        - conversation_id and turn_number
        ...
        print(f"\n{Colors.DIM}🔍 DEBUG: Message Extraction for Turn {memory.turn_number}{Colors.END}")
        print(f"{Colors.DIM}   Total messages: {len(messages_for_breakdown)}{Colors.END}")
        for i, msg in enumerate(messages_for_breakdown):
            role = msg.get('role', 'unknown')
            content = msg.get('content', '')
            est_tokens = len(content) // 4
            # ✅ ADD THIS: Show content preview
            ...
            print(f"{Colors.DIM}       Preview: {preview}{Colors.END}")  # ✅ NEW LINE
        ...
        # ⭐ Create prompt breakdown with chat history (FIXED!)
        prompt_breakdown = create_prompt_breakdown_from_messages(messages_for_breakdown)
        
        # ✅ NEW: Extract tool definition tokens automatically
        ...
        # ✅ UPDATED: Show breakdown results including tools
        if prompt_breakdown:
            print(f"{Colors.DIM}   📊 Breakdown: sys={prompt_breakdown.system_prompt_tokens or 0}, "
                  f"user={prompt_breakdown.user_message_tokens or 0}, "
        ...
        try:
            ...
            # ⭐ Create prompt breakdown with chat history (FIXED - now includes chat_history_count!)
            prompt_breakdown = create_prompt_breakdown_from_messages(messages_for_breakdown)
            ...
            track_kwargs = {
                ...
                # Prompt breakdown (includes chat_history AND chat_history_count - FIXED!)
                "prompt_breakdown": prompt_breakdown,
                "system_prompt_tokens": system_prompt_tokens,  # ✅ ADD
                "user_message_tokens": user_message_tokens,    # ✅ ADD
                "chat_history_tokens": chat_history_tokens,    # ✅ ADD
